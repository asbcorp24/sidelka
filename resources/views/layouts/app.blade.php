...
<head>
    ...
    @stack('styles')
    <style>
        :root { --brand:#1f6f78; --brand-dark:#174f56; --sand:#f4efe7; --mint:#d7ebe5; --ink:#213547; --muted:#6c7a86; --card-shadow:0 18px 45px rgba(19,46,61,.12); }
        body { font-family:'Manrope',sans-serif; color:var(--ink); background:radial-gradient(circle at top right,rgba(215,235,229,.85),transparent 28%),linear-gradient(180deg,#f9f6f1 0%,#fff 100%); }
        .navbar-brand { font-weight:800; letter-spacing:.03em; }
        .hero-shell,.card-soft { border:1px solid rgba(31,111,120,.08); box-shadow:var(--card-shadow); }
        .hero-shell { background:linear-gradient(135deg,rgba(244,239,231,.95),rgba(215,235,229,.95)); border-radius:28px; }
        .card-soft { border-radius:22px; background:rgba(255,255,255,.92); }
        .badge-soft { background:rgba(31,111,120,.1); color:var(--brand-dark); }
        ...
        .availability-chip,.service-chip { display:inline-flex;align-items:center;gap:.35rem;border-radius:999px;padding:.45rem .8rem;background:#f4f8f9;color:var(--ink);font-size:.92rem;margin:0 .45rem .45rem 0; }
        .metric { background:#fff;border-radius:20px;padding:1.2rem;height:100%;border:1px solid rgba(31,111,120,.08); }
        ...
        <div class="collapse navbar-collapse" id="navMain">
            <ul class="navbar-nav ms-auto align-items-lg-center gap-lg-2">
                ...
                @auth
                    ...
                    @elseif(auth()->user()->isCrm() || auth()->user()->isAdmin())
                        <li class="nav-item"><a class="btn btn-dark rounded-pill px-4" href="{{ route('crm.dashboard') }}">CRM</a></li>
                        @if(auth()->user()->hasStaffPermission('crm.requests.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.people.index') }}">Люди</a></li>@endif
                        @if(auth()->user()->hasStaffPermission('crm.documents.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.caregiver-documents.index') }}">Допуски</a></li>@endif
                        @if(auth()->user()->hasStaffPermission('crm.disputes.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.shift-disputes.index') }}">Споры</a></li>@endif
                        @if(auth()->user()->hasStaffPermission('crm.incidents.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.incidents.index') }}">Инциденты</a></li>@endif
                        @if(auth()->user()->hasStaffPermission('crm.contracts.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.contracts.index') }}">Договоры</a></li>@endif
                        @if(auth()->user()->hasStaffPermission('crm.finance.manage'))<li class="nav-item"><a class="nav-link" href="{{ route('crm.finance.index') }}">Финансы</a></li>@endif
...
<main class="pb-5">
    <div class="container">
        @if(session('status'))<div class="alert alert-success rounded-4 mt-3 mb-0">{{ session('status') }}</div>@endif
        @if($errors->any())<div class="alert alert-danger rounded-4 mt-3 mb-0"><ul class="mb-0 ps-3">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
    </div>
