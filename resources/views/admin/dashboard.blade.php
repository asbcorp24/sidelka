...
<div class="container py-4 py-lg-5">
    ...
    <div class="row g-4 mb-4">
        <div class="col-md-3"><div class="metric"><div class="value">{{ $stats['caregivers'] }}</div><div>Сиделок</div></div></div>
        <div class="col-md-3"><div class="metric"><div class="value">{{ $stats['clients'] }}</div><div>Клиентов</div></div></div>
        <div class="col-md-3"><div class="metric"><div class="value">{{ $stats['verified_caregivers'] }}</div><div>Проверенных сиделок</div></div></div>
        <div class="col-md-3"><div class="metric"><div class="value">{{ $stats['news_posts'] }}</div><div>Новостей</div></div></div>
    </div>
        ...
        <div class="col-lg-4">
            <div class="card-soft p-4 mb-4">
                ...
                <form action="{{ route('admin.services.store') }}" method="POST" class="row g-3">
                    ...
                    <div class="col-12">
                        <input type="number" name="hourly_surcharge" class="form-control" placeholder="Доплата, ₽/час">
                    </div>
                    <div class="col-12">
                    ...
                    <div class="col-12">
                        <button class="btn btn-dark rounded-pill px-4">Сохранить услугу</button>
                    </div>
                </form>
            </div>
            ...
            <div class="card-soft p-4">
                ...
                <form action="{{ route('admin.news.store') }}" method="POST" class="row g-3">
                    ...
                    <div class="col-12">
                        <textarea name="excerpt" class="form-control" rows="2" placeholder="Краткое описание"></textarea>
                    </div>
                    <div class="col-12">
                        <textarea name="body" class="form-control" rows="4" placeholder="Текст новости"></textarea>
                    </div>
                    <div class="col-12">
                        <div class="form-check">
                    ...
                    <div class="col-12">
                        <button class="btn btn-dark rounded-pill px-4">Сохранить новость</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <form action="{{ route('admin.users.update', $platformUser) }}" method="POST" class="border rounded-4 p-3 mb-3">
                        ...
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                            <div>
                                ...
                                <div class="text-secondary">{{ $platformUser->email }} • {{ $platformUser->city }}</div>
                            </div>
                        ...
                        <div class="row g-3 mt-1">
                            <div class="col-md-6">
                                <select name="role" class="form-select">
                                    ...
                                    <option value="admin" {{ $platformUser->role === 'admin' ? 'selected' : '' }}>Админ</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-2">
                                <button class="btn btn-outline-dark w-100">Обновить</button>
                            </div>
                        </div>
                    </form>
                @endforeach
