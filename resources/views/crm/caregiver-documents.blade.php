...
<div class="container-fluid px-3 px-xl-5 py-4">
    <div class="d-flex justify-content-between align-items-end flex-wrap gap-3 mb-4"><div><div class="text-uppercase small text-secondary">Допуск к сменам</div><h1 class="section-title mb-0">Документы сиделок</h1></div><form class="d-flex gap-2"><select name="status" class="form-select"><option value="">Все документы</option><option value="expired">Просроченные</option><option value="expiring">Истекают за 30 дней</option><option value="unverified">Не проверены</option></select><button class="btn btn-dark">Показать</button></form></div>
    <div class="row g-3 mb-4"><div class="col-md-4"><div class="metric"><div class="value text-danger">{{ $stats['expired'] }}</div><div>Просрочено</div></div></div><div class="col-md-4"><div class="metric"><div class="value text-warning">{{ $stats['expiring'] }}</div><div>Истекает за 30 дней</div></div></div><div class="col-md-4"><div class="metric"><div class="value">{{ $stats['blocking'] }}</div><div>Документов с блокировкой</div></div></div></div>
